...
extension MainTabViewModel {
    ...
    func saveCurrentImageToUserDefaults(_ currentImage: UnsplashImage) async throws {
        do {
            try defaults.saveModel(key: .currentImageKey, value: currentImage)
    ...
    func getCurrentImageFromUserDefaults() async {
        do {
            let image: UnsplashImage? = try defaults.getModel(key: .currentImageKey, type: UnsplashImage.self)
